      ...
      <ol aria-busy={!list.length && !error}>
        ...
        {#each shown as f (f.code_point)}
          <li><button class:current={f.code_point === code} onclick={() => pick(f.code_point)}>
            ...
            <span class="family-progress" aria-label={t('forms.percentDone', { count: Math.round(100 * (f.assigned + f.rejected) / f.count) })}><i style={`width:${100 * f.assigned / f.count}%`}></i><i class="rejected" style={`width:${100 * f.rejected / f.count}%`}></i></span>
          </button></li>
        {/each}
      </ol>
    </aside>
