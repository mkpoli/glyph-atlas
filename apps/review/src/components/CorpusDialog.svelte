<script>
  ...
  const sourceName = $derived(data?.source?.corpus === 'codh-full' ? 'CODH' : data?.source?.corpus || 'Corpus')
...
<dialog class="character-dialog corpus-dialog" bind:this={dialog} oncancel={close} onclick={e => { if (e.target === dialog) close() }} aria-label="Corpus character reviewer">
  <div class="inspector">
    ...
    {#if data}
      ...
      <div class="inspector-title"><h2 class:unassigned-title={data.identity_status === 'unassigned'}>{#if data.identity_status === 'unassigned'}Unassigned{:else}<ReferenceGlyph char={data.written_character ?? data.label} code_point={data.code_point} size="lg" />{/if}</h2>{#if data.identity_status !== 'unassigned'}<ZiLink character={data.written_character ?? data.label} />{/if}<span class="state-pill" class:flagged={data.state === 'flagged'}>{data.needs_segmentation ? 'Needs splitting' : data.state === 'checked' ? 'Checked here' : data.state === 'flagged' ? 'Flagged' : data.state === 'stale' ? 'Source changed' : 'Unreviewed'}</span></div>
      <p class="corpus-source-label">{#if data.needs_segmentation}<span>{data.character_count} characters · </span>{/if}{sourceName} label <b>{data.source_label}</b> <ZiLink character={data.source_label} compact />{#if data.identity_status !== 'unassigned' && data.label !== data.source_label}<span> → <b>{data.label}</b> · Atlas correction</span>{/if}</p>
      <div class="inspector-figure">
        ...
        {:else}<span>Image unavailable</span>{/if}
      </div>
      {#if data.identity_status === 'unassigned'}
        <div class="assignment-options" aria-label="Assign written character">
