...
<span class="script-text" role="img" aria-label={description}>{#each parts as part, index (index)}<span class="script-char" data-script={part.key} lang={part.key === 'hangul' || part.key === 'gugyeol' ? 'ko' : 'ja'} title={`${part.text} · ${part.label}`} aria-hidden="true">{part.text}</span>{/each}</span>
