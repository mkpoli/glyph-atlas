...
try {
  ...
  await settled()
  assert(await material() === 'non-movable-type', 'initial review included movable type')
  assert(!await browser.evaluate('!!document.querySelector(".quiz-tile [data-production=movable-type]")'), 'movable type in default queue')
  ...
  await choose('movable-type')
  await browser.waitFor('document.querySelectorAll(".quiz-tile").length > 0')
  assert(await browser.evaluate('[...document.querySelectorAll(".quiz-tile [data-production]")].every(t => t.dataset.production === "movable-type")'), 'explicit movable-type queue is mixed')
  const start = requests.length
  if (await browser.evaluate('!!document.querySelector(".load-more:not(:disabled)")')) await click('.load-more')
  await settled()
  assert(requests.slice(start).every(scope => scope === 'movable-type'), 'load more escaped material filter')
  await click('.quiz-choice:not(:disabled)')
  ...
  assert(await material() === 'non-movable-type', 'history lost original material filter')
  ...
  assert(await material() === 'movable-type', 'explicit return lost printed material')
  ...
  assert(!await browser.evaluate('!!document.querySelector(".quiz-tile [data-production=movable-type]")'), 'old printed crops remained after filtering')
  await choose('non-movable-type')
  await browser.waitFor('document.querySelectorAll(".quiz-tile").length > 0')
  assert(!await browser.evaluate('!!document.querySelector(".quiz-tile [data-production=movable-type]")'), 'return to default leaked printed crops')
