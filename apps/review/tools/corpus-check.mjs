...
try {
  ...
  await browser.goto(service.base + '/', { waitFor: `document.querySelector(${JSON.stringify(tile)})` })
  ...
  assert(await browser.evaluate(`location.pathname === '/' && document.querySelector('dialog').open`), 'tile left the app')
  ...
  await browser.waitFor(`!document.querySelector('.corpus-dialog') || !document.querySelector('.corpus-source-label')?.textContent.includes('Atlas correction')`)
  await Bun.sleep(400)
  ...
  await browser.evaluate(`visit('/corpus/' + encodeURIComponent(${JSON.stringify(fixture.next)}))`)
  await browser.waitFor(`document.querySelector('.corpus-dialog .save-character')?.disabled === false`)
  await click('.skip-character')
