...
try {
  ...
  await check('standalone reviewer uses one viewport and keeps manual crop adjustment explicit', async () => {
    ...
    await browser.evaluate(`location.hash = '#/character/' + encodeURIComponent(${JSON.stringify(selected[0])})`)
  ...
  await check('corpus reviewer shares the viewport without treating provenance as page pixels', async () => {
    ...
    await browser.evaluate(`location.hash = '#/corpus/' + encodeURIComponent('codh:200008316:200008316_00030_2:B0001:C0027')`)
