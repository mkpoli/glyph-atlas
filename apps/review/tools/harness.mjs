#!/usr/bin/env bun
/**
 * Shared scaffolding for the two tools that need a running service: a fixture dataset, the T40
 * service over it, and a way to read back the events it recorded.
 *
...
export async function boot(config) {
  ...
  if (!config.external) {
    ...
    server = Bun.spawn(
      [config.python, '-m', 'kuzushiji_atlas.cli', 'review', 'serve', config.directory, '--port', String(config.port)],
      {
        cwd: ROOT,
        env: {
          ...process.env,
