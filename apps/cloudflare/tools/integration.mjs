...
try {
  ...
  await db.prepare('INSERT INTO unit_suspects VALUES(?,?,?)').bind('two', 0.01, 'ア').run()
  const marked = Object.fromEntries((await call('/atlas?purpose=review&production=all')).items.map(i => [i.id, i.suspect]))
  assert.deepEqual(marked, { one: null, two: { p: 0.01, reads_as: 'ア' } }, 'a crop carries the classifier\'s doubt, or null without one')
  ...
  await db.prepare('INSERT INTO unit_suspects VALUES(?,?,?)').bind('na-3', 0.02, null).run()
  ...
  assert.equal(doubted['na-1'], null)
  const paged = []
