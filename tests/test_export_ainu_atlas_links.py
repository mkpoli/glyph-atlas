...
def fixture(tmp_path: Path, *, atlas_width: int = 1000) -> tuple[Path, Path, Path]:
    ...
    samples = {"key": "moshiogusa/ninjal-1", "pages": [{"n": 1, "width": 1000, "height": 800}],
               "samples": [{"id": "1-l1-0", "page": 1, "box": [100, 100, 40, 50]},
                           {"id": "1-l1-1", "page": 1, "box": [100, 160, 40, 50]},
                           {"id": "1-l1-2", "page": 1, "box": [300, 300, 40, 50]}]}
    ...
    with sqlite3.connect(catalogue) as db:
        ...
        for uid, box in [(f"hk:{ENTRY}:0:L0:r:1", {"x": 101, "y": 101, "w": 40, "h": 50}),  # the same ink
                         (f"hk:{ENTRY}:0:L0:r:2", {"x": 100, "y": 190, "w": 40, "h": 50}),  # overlaps 1-l1-1 weakly
                         (f"hk:{ENTRY}:1:L0:r:1", {"x": 300, "y": 300, "w": 40, "h": 50})]:  # another page
            page = ":".join(uid.split(":")[:3])
            db.execute("INSERT INTO units VALUES (?, 'local', ?)", (uid, json.dumps({"id": uid, "page_id": page, "box": box})))
