...
READY = {"status": "ready", "candidates": [{"text": "天"}]}
...
class Model:
    ...
    def __init__(self, provider="CUDAExecutionProvider"):
        self.engines = [{"name": "NDLkotenOCR", "sha256": "x", "provider": provider}]

    def read(self, image):
        Model.reads += 1
        return {"status": "ready", "candidates": [{"text": "大"}]}
...
def test_unread_crops_are_read_once_and_cached(export, monkeypatch):
    ...
    module.read_crops(db, media)
    assert Model.reads == 1 and visual(db, "unread")["status"] == "ready"
