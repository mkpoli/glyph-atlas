...
from glyph_atlas import align, tables
from glyph_atlas.schema import Box, Line, Page, Unit, UnitKind
...
DETECTOR = Path(__file__).resolve().parents[1] / "models" / "detector" / "artifacts" / "detector.onnx"
needs_detector = pytest.mark.skipif(not DETECTOR.exists(), reason="needs the trained detector")
...
def test_the_run_carries_the_detectors_operating_point(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("glyph_atlas.detect.Detector", spy)
    page = Page(id="hl:one:0", document_id="doc", seq=0, canvas="c", image="i", width=10, height=10)
    tables.write(tmp_path / "pages.parquet", [page], Page)
...
def test_a_run_carries_the_detectors_suppression(tmp_path: Path, monkeypatch) -> None:
    ...
    """
    seen: dict[str, float] = {}
    from glyph_atlas.detect import Detector as RealDetector
