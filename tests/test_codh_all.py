...
`tmp_path` through `KUZUSHIJI_ATLAS_CACHE`, the book list and the split file at fixtures, so nothing
...
from kuzushiji_atlas.importers import codh_all
from kuzushiji_atlas.schema import Classification, Production, ReviewState, UnitKind
...
def test_register_images_without_pages_needs_the_split_file(tmp_path, cache, books_file, zips, monkeypatch):
    fixture_import(tmp_path, books=["900000001", "900000002"])
    # the split file of T20 is not written yet, and the test must not read the one in the repository
