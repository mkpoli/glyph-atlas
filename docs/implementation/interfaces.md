...
Task cards state the outputs. This page fixes the names and signatures that more than one card
calls, so that cards written in parallel meet. A card may add functions; it changes nothing here
without saying so in its pull request.

## Layout

- Python package `src/kuzushiji_atlas/`, tests `tests/`, scripts `scripts/`, model code
  `models/<name>/`.
...
`atlas` is one Typer application in `src/kuzushiji_atlas/cli.py` with sub-applications `tables`,
`images`, `rights`, `import`, `pilot`, `eval`, `review`, `audit`, `align`, `jibo`, `export` and the
top-level commands `sources` and `coverage`. A card that needs a new command adds a module under
`src/kuzushiji_atlas/` and registers it in `cli.py`; it does not put logic in `cli.py`.
