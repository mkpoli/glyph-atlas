...
Kuzushiji Atlas is a character-shape dataset of pre-modern Japanese writing. One record is one
...
The dataset is created and maintained by mkpoli at https://github.com/mkpoli/kuzushiji-atlas. The
...
before adjudication, minutes per page and disagreement categories are reported in
`docs/reports/pilot-calibration.md`. A blind random sample of accepted machine units gives the
published precision, reported in `docs/reports/audit-*.md`.
...
units. Precision of the accepted units is measured on a blind random sample and reported in
...
- A box can sit on the wrong ink: forced alignment places a box on a detected region when a
  ...
  units is in `docs/reports/audit-*.md`.
...
https://github.com/mkpoli/kuzushiji-atlas/issues.
