...
character's `variants` lists the `mj` scheme entries for every figure its code point has, several
where the table has several. `data/vocab/graphemes.yaml` states the curated kana assignments, 字母
