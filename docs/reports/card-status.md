...
| T15 みんなで翻刻 data | done | 1,558 of 7,587 entry manifests are unreadable, counted by host: 1,151 sit behind 資料編纂所 (HTTP 500 every time) and a dead khirin-labs name, 243 behind Gallica's rate limit, 103 gone from honkoku.org. The first group no retry reaches; `scripts/retry_manifests.py` retries only the rest and skips the refusers, and Gallica needs a pace of tens of seconds an entry |
