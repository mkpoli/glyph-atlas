"""Verify live ONNX feature/probability parity on prepared real crops."""
import argparse
import hashlib
import json
...
def main(*, disable_tf32=False):
    ...
    models = []
    for name in ("classifier.onnx", "classifier-with-features.onnx"):
        path = Path("models/classifier/artifacts") / name
        ...
        models.append(Classifier(path, classes="models/classifier/classes.json", session=session))
    baseline, extended = models
    model_hashes = {}
    for model in models:
        with model.onnx_path.open("rb") as handle:
            model_hashes[model.onnx_path.name] = hashlib.file_digest(handle, "sha256").hexdigest()
    comparisons = []
    ...
    for index in indexes:
        ...
        with Image.open(path) as image:
            features = extended.features(image)
            original = baseline.probabilities(image)
            current = extended.probabilities(image)
        if features is None:
        ...
        comparisons.append({"id": row["id"], "cosine": float(features @ vectors[index]),
                            ...
                            "probability_max_abs": float(np.max(np.abs(original - current))),
                            "top_class_equal": bool(original.argmax() == current.argmax())})
    ...
    report = {"samples": len(comparisons), "device": "CUDAExecutionProvider", "tf32": not disable_tf32,
              ...
              "passed": all(row["cosine"] >= .99999 and row["feature_max_abs"] < feature_tolerance
                            and row["probability_max_abs"] < 1e-5 and row["top_class_equal"]
    ...
    if not report["passed"]:
        raise ValueError("ONNX feature or baseline probability parity failed")
