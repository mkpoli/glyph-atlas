...
from glyph_atlas.schema import Box, Document, Unit

ROOT = Path(__file__).resolve().parents[1]

#: Tile geometry, the same one `glyph_atlas.detect` cuts pages with at inference time.
TILE = 1024
OVERLAP = 128
STRIDE = TILE - OVERLAP
PAD = 255
...
def print_report(report: Report, seconds: float) -> None:
    """The numbers the acceptance target names, then the per-split table."""
    options = report.options
