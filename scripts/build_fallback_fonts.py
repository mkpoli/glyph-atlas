...
def labels(parquet):
    ...
    rows = [r for r in pq.read_table(parquet, columns=['char', 'n_units']).to_pylist() if r['n_units'] > 0 and len(r['char']) == 1]
...
def main():
    ...
    print('\n'.join(report))
    print(f'{len(need)} labels drawn by none of these fonts:', ''.join(ch for ch, _ in need[:60]))
