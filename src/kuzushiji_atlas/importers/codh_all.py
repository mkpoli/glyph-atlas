...
validation books of `data/splits/codh.tsv` (T20) and of a sample of its train books. Any other page
...
#: The evaluation split by book, written by T20: bid, production, split.
...
def split_bids(path: Path | None = None) -> list[tuple[str, str]]:
    ...
    The file is `data/splits/codh.tsv` of T20: `bid`, `title`, `production` and `split`,
    tab-separated under a header line, with comment lines above it. A file without a header is read
    as the card states it, `bid`, `production` and `split`.
    """
