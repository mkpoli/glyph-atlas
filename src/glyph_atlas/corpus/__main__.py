"""``python -m glyph_atlas.corpus`` — build and query the occurrence index.
...
``cli.py``; a one-line hook there (``app.add_typer``) can adopt it later.
    ...
    python -m glyph_atlas.corpus build   --root shared-work --out work/corpus-index
    python -m glyph_atlas.corpus find    𪜈 --limit 20
    python -m glyph_atlas.corpus find    U+2A708 --located --format jsonl
    python -m glyph_atlas.corpus chars   --limit 40
    python -m glyph_atlas.corpus stats
    python -m glyph_atlas.corpus serve-request "/api/corpus/find?char=𪜈&limit=3"
    python -m glyph_atlas.corpus wikisource --search 𪜈
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m glyph_atlas.corpus")
