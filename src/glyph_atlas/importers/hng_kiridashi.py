...
A unit whose hng-gid is set names the crop of the basic dataset it was filed under in
`upstream["hng_unit"]`: `hng:{id}:{gid}` with the letter of flg where the 字体 is specified.
"""
...
def records_of(sheet: Sheet, document: Document, counts: Counter[str]) -> tuple[list[Page], list[Line], list[Unit]]:
    ...
    for row in sheet.rows:
        ...
        if page is None:
            ...
            pages[row["page"]] = page
        column = int(row["ln"])
        unit = unit_of(row, sheet, page, column)
...
def unit_of(row: dict[str, str], sheet: Sheet, page: Page, column: int) -> Unit:
    ...
    if row["hng-gid"]:
        flag = int(row["flg"] or 0)
        letter = "abcdefghij"[flag - 1] if 0 < flag <= 10 else ""
        upstream["hng_gid"] = row["hng-gid"]
        upstream["hng_unit"] = f"hng:{sheet.source}:{row['hng-gid']}{letter}"
    if row["remarks"]:
...
def import_all(out: Path, *, clone: Path | None = None, limit: int | None = None) -> dict[str, int]:
    """Import every TSV of the clone into `out` and return the row counts and the duplicates.

    Raises `RevisionError` when the clone is a git checkout at another commit than the source file
    pins, and `KeyError` when a sheet names an HNG source the basic dataset's source file lacks.
    """
    ...
    lines: list[Line] = []
    units: list[Unit] = []
    for path in sorted(clone.glob("H*.tsv")):
        ...
        sheet_pages, sheet_lines, sheet_units = records_of(sheet, document, counts)
