...
A page is cut into 1024x1024 tiles with 128 px of overlap, the geometry the detector's training data
build uses, and the tiles at the right and bottom edge are padded with white. A detection is
...
# Tiling geometry, the same one the detector's training data build cuts training tiles with.
TILE = 1024
OVERLAP = 128
STRIDE = TILE - OVERLAP
PAD = 255
