"""Fine-tune the classifier on the HI Lab crops. A separate experiment, never the baseline.

`work/codh-full` is one dataset of characters on pages, cut by CODH's own boxes, split by book. The
東京大学史料編纂所 くずし字データセット (`work/hilab/units.parquet`) is another: 325,261 crops
of individual characters, several hands, cropped and labelled by the holder, with no page placement
and no book to split by. They are kept apart, so this script never touches
...
#: Where the HI Lab import puts the extracted crops, below the root of the archive.
...
def build(
    ...
    *,
    ...
    """Write `hilab-train.parquet` and `hilab-val.parquet` from the HI Lab units that are on disk."""
