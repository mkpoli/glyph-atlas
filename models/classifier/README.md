...
Calibrated probabilities over code points for one character crop, used by the alignment of T23 to
score a detection against the character the transcription holds there. `kuzushiji_atlas.classify`
...
unit, cut to the box CODH drew, from the pages T20 already cached and the tiles T20 materialised.
...
the page, the split and the production type. It reads `work/codh-full` (T10) and the split table
`data/splits/codh.tsv` (T20), which splits by book, so a hand, a block and a scan stay on one side of
...
or from the checksum cache through `kuzushiji_atlas.images`.

Crops are grey JPEGs under `work/classifier/crops/<split>/<bucket>/<unit id>.jpg`, cut exactly to the
unit's box: the classifier is measured on CODH boxes and serves T21's boxes, so no margin is added
and nothing is resized at this step. The preprocessing that resizes is `kuzushiji_atlas.classify`,
...
the model's output, and `kuzushiji_atlas.classify` reads it from beside the export (or one directory
...
A character mirrored or rotated is a different character, so neither is used, as in T21. The sampler
...
Against the card: top-1 0.9348 is above the 0.93 asked for and the calibration error 0.0151 is below
...
The classifier needed its own spike line, because the card's framework is not the detector's. What
...
- One training step at batch 64 on 96×96 bf16 takes 0.035 s and peaks at 1.21 GiB, so the card's
...
That is what the card's preprocessing fixes, and the metrics below are measured at it.

## HI Lab crops

`train_with_hilab.py` is a separate experiment on the 東京大学史料編纂所 くずし字データセット (T12),
...
Not run: T12's archive is imported as a listing only, the crops are not extracted, and this machine
...
```python
from kuzushiji_atlas.classify import Classifier
...
data they were trained on, as T21's weights are. Whether trained weights are a derivative work of
their training data is not settled in any jurisdiction, so the release states the intent, names the
data, and carries the attribution T50 generates. Weights go to a GitHub release, not to git.
