...
The card time-boxes a day for this: try mmdetection with RTMDet-s against the PyTorch build the GPU
...
**Verdict: mmdetection with RTMDet-s is not viable within the card's time box on this machine.**
...
card's three acceptance items, each run on the card's hardware:
...
1200x1500 with 50 px black squares, cut into 24 tiles of 1024² with the T20 geometry, written as
...
```sh
.venv/bin/python -c "
from PIL import Image; from kuzushiji_atlas.detect import Detector
...
out for this card: no `mmcv` wheel and no source build that configures, compiles and would then run
on Python 3.14 with a current PyTorch.

Nothing in the spike was blocked by the missing T20 data; `train.py`, `export_onnx.py`, `detect.py`
