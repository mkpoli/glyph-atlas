...
boxes and exported to ONNX for `glyph_atlas.detect`. It proposes boxes inside a line; the
transcription is aligned to them by the alignment and a character classifier says which character
each one holds.
...
`work/detector/{train,val,test}.json` from `scripts/build_detector_data.py`, COCO with one
...
- Ruby is not annotated by CODH, so it is background during training and a false positive during
  measurement. The metrics exclude a detection inside a known ruby line box; those boxes are not in
  the training data build's split, so that exclusion is not wired yet, and the ignore regions that
  are wired are the two kinds the split marks `iscrowd`.
...
more boxes than `num_queries`, which is the one number the training data build's statistics set.
...
built, before the training data build's data existed; the second is the real run over the training
split:
...
| Wall clock, whole session | 5 h 25 min, of which about 1 h was spent on a run abandoned at epoch 2 and on a first 24-epoch-scheduled attempt that the time budget stopped |
...
The target is recall ≥ 0.95 and precision ≥ 0.95 at IoU 0.5 on the printed `test` books. It is a
joint target: raising precision by lowering recall does not meet it, and vice versa.

Measured on the whole `test` split at three suppressions, the third chosen on `val` by the sweep in
`models/detector/sweep_nms.py` (results in `models/detector/nms-sweep.json`):
...
The suppression is the largest single improvement measured here: the duplicate and split false
...
setting is 0.8533, below the target's 0.95**, where the shipped setting met the recall bar (0.9283) and
...
- CODH does not annotate ruby, punctuation or decoration, and 74% of the false positives sit on ink
  with no box near it. The metrics exclude detections inside known ruby line boxes; the training
  data build's split carries no ruby boxes, so that exclusion is not in these numbers and the true
...
`Detector` cuts the page into the tiles the training data build trained on, runs them on
...
`src/glyph_atlas/detect.py` is the only reader of the export. The versions it was checked
...
the export step generates. Weights go to a GitHub release, not to git.
