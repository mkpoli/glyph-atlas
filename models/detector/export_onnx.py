...
form normalised to the tile. That is the layout `glyph_atlas.detect.Detector` decodes, and a
...
difference: the target is the same boxes within 1 px on ten pages. It reads the tiles of the
`val` split, so it is skipped with a message while the training data build's data is not there.
"""
...
def build(checkpoint: Path | None, device: torch.device, config_path: Path) -> tuple[Any, dict]:
    ...
    detector and so a subject for the parity check before this detector's training has run.
    """
